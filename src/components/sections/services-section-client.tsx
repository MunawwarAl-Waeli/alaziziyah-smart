...
// ======================================
// EXTREME MODE - Ultra Lightweight Grid
// ======================================
export function ServicesGridClient({
  ...
  // نقسم البيانات لتقليل الحمل
  const displayServices = useMemo(() => {
    return limit ? services.slice(0, limit) : services;
  }, [services, limit]);

  // 👇 progressive rendering (أهم نقطة)
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          // نبدأ تحميل تدريجي بدل دفعة واحدة
          let i = 0;
          const chunkSize = 3;
          ...
          const interval = setInterval(() => {
            ...
            setVisibleCount((prev) => {
              ...
              return next > displayServices.length
                ? displayServices.length
                : next;
            });

            if (i >= displayServices.length) {
              clearInterval(interval);
            }
          }, 80); // throttle خفيف جدًا
  ...
  const visibleServices = displayServices.slice(0, visibleCount);

  return (
    <section
      ref={sectionRef}
      className="relative py-20 md:py-28 bg-gradient-to-b from-background via-background/95 to-slate-50 dark:to-slate-950"
      dir="rtl"
    >
      <div className="container mx-auto px-4">
        ...
        {visibleServices.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-7">
            {visibleServices.map((service) => (
              <div key={service.id} className="will-change-transform">
        ...
        {/* ================= CTA ================= */}
        {visibleCount >= displayServices.length &&
          services.length > (limit || 0) && (
            <div className="mt-14 md:mt-16 text-center">
              <Link
                href="/services"
                className="group inline-flex items-center gap-3 px-7 py-4 bg-gradient-to-l from-amber-600 to-amber-500 text-white rounded-2xl font-bold shadow-md"
              >
