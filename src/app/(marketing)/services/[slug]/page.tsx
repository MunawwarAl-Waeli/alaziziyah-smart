...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonicalUrl = `https://al-azizia.com/services/${cleanSlug}`;
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: service.featuredImage?.node?.sourceUrl || "https://al-azizia.com/1.jpg",
          width: 1200,
          height: 630,
        }
      ],
...
export default async function ServiceDetailPage({ params }: Props) {
  ...
  const sSchema = serviceSchema({
    ...
    image: service.featuredImage?.node?.sourceUrl || "https://al-azizia.com/1.jpg",
  ...
  const currentCategorySlugs = service.serviceCategories?.nodes.map((n) => n.slug) || [];
  ...
  if (relatedServices.length === 0) {
    relatedServices = allServices.filter((s) => s.slug !== service.slug).slice(0, 4);
  } else {
