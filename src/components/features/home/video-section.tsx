...
import { Play, X, Clock, Video, ChevronLeft, ChevronRight } from "lucide-react";
...
export function VideoGallery({ initialVideos, error }: VideoGalleryProps) {
  ...
  const scroll = (direction: "left" | "right") => {
    if (scrollRef.current) {
      ...
      // التمرير بعرض العنصر الكامل لتغيير العمود بشكل كامل
      ...
      <div className="container mx-auto relative z-10 px-4">
        ...
        <div className="text-center mb-10 max-w-2xl mx-auto">
          ...
          <p className="text-muted-foreground text-sm md:text-base leading-relaxed">
            اكتشف دقة التصنيع واحترافية التركيب. انقر على أي فيديو لمشاهدة
            التفاصيل بالصوت والصورة.
          </p>
        </div>

        {/* أزرار التنقل السريع */}
        <div className="flex justify-end gap-2 mb-4 max-w-6xl mx-auto px-2">
          <button
            onClick={() => scroll("right")}
            className="w-10 h-10 rounded-full bg-primary/10 hover:bg-primary text-primary hover:text-white flex items-center justify-center transition-colors border border-primary/20 shadow-sm"
            aria-label="التالي"
          >
        ...
        {/* حاوية الفيديوهات (عمود واحد للجوال، وعناصر منسقة بدقة) */}
        <div
          ref={scrollRef}
          className="flex gap-6 overflow-x-auto pb-6 pt-2 px-2 snap-x snap-mandatory scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] max-w-6xl mx-auto"
        >
          {initialVideos.map((video, index) => (
            <div
              key={video.id}
              onClick={() => setPlayingVideo(video)}
              className="relative rounded-3xl overflow-hidden cursor-pointer shrink-0 snap-center group border shadow-xl bg-card border-border w-full sm:w-[48%] lg:w-[32%] h-[400px] md:h-[430px] transition-transform duration-300 active:scale-[0.99]"
            >
              {/* الصورة */}
              <Image
                src={video.thumbnail}
